...
public class ReservationManager {
    ...
    public void cancelAllReservation() {
        ...
        if(user == null || user.getReservedMovies().isEmpty()) {
            System.out.println(userName + "남은 예약된 내역이 없습니다.");
            return;
        }
    ...
    public void printTicket() {
        ...
        int reservationNum = -1; // 예매번호를 초기화
        try {
    ...
    public void deleteMovie(Scanner sc) {
        ...
        String title = sc.nextLine();
        Movie movie = getMovie(title); // 삭제할 영화제목은 title
        if(movie != null){
            ...
            for(User user : users){
              ...
              if(reservedMovies.contains(title)){
                //예매정보 인덱스 번호
                ArrayList<Integer> reservationNumbers = new ArrayList<>(user.getReservationNumbers()); // 예매번호를 모두 가져옴
    ...
    public void modifyMovieInfo(Scanner sc2) {
        ...
        try {
            if(priceInput.trim().isEmpty()){ 
                //공란을 입력시 기존 가격 유지 (enter)
                newPrice = movie.getPrice();
            }else{
                newPrice = Integer.parseInt(priceInput); // 가
            }

        } catch (Exception e) {
        System.out.println("잘못된입력입니다s. 가격은 숫자여야합니다.");
        }
